...
class Grn extends CI_Controller {
	...
	function daftar($act="", $id="") {
		...
		if(in_array($act, array("add","edit","preview","realisasi"))) {
			$this->proccess("grn", $act, $id);
		} else {
			$this->breadcrumbs = array("title"=>"Good Receive Note","icon"=>"sign-in","title_child"=>"Daftar","url"=>'grn/daftar');
			$this->load->model('m_table','model');
			$arrdata = $this->model->tpb_hdr("grn",$act);
	...
	function details($act, $id="") {
		...
		if (in_array($act, array("add","edit"))) {
			...
			$this->load->model('m_table','model');
			$arrdata = $this->model->details("grn", $act, $id);
	...
	function proccess($type, $act, $id="") {
		...
		if ($act == "add") {
			...
			if ($type == "details") {
				...
				$arrdata['placeholder'] = "Exp. No. Inveoice / No. Purchase Orders";
				$arrdata['title'] = "Partner";
				$arrdata['tipe'] = $type;
				...
				if ($this->session->userdata('TIPE_DOKUMEN') == "1") {
					$arrdata['combo_dokumen'] = $this->model->get_data("reff_dokumen","IN");
				}
			...
			if ($type == "details") {
				if ($this->input->post("tb_chktblgrndtl")) {
					list($id) = $this->input->post("tb_chktblgrndtl");
				...
				if ($this->input->post("tb_chktblgrnhdr")) {
					list($id) = $this->input->post("tb_chktblgrnhdr");
			...
			$this->breadcrumbs = array("title"=>"Good Receive Note","icon"=>"sign-in","title_child"=>"Daftar","url_2"=>'grn/daftar/'.$type,'title_child_2'=>ucwords($act));
